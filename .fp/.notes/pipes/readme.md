...
- `stdin` `stdout` `stderr` 每个进程都有的三个东西，管道就是对接前者输出到后者输入。
- `|` 一般的管道。
- `&|` 算是 Bash 的语法糖，大体相当于 `... 2>&1 | ...` 的效果。
- `<()` 和 `>()` 用于创建匿名文件，它们就是匿名文件名，任何可疑填文件名的地方都可以填它们。
- `File` 是可 IO 对象，是一种对 IO 活动的封装方案的一部分，其提供的文件名就是 IO 的接口。它不一定指硬盘上的持久化存储。
